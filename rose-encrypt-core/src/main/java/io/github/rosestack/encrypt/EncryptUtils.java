...
import io.github.rosestack.util.MapUtils;
...
public final class EncryptUtils {
     ...
     * 安全的加密算法配置
     */
    private static final Map<EncryptType, String> TRANSFORMATIONS = MapUtils.of(
     ...
     * IV 长度配置
     */
    private static final Map<EncryptType, Integer> IV_LENGTHS = MapUtils.of(
     ...
     * 监控管理器
     */
    private static final EncryptMonitorManager MONITORING_MANAGER = EncryptMonitorManager.getInstance();
    ...
    public static String encrypt(String plainText, EncryptType encryptType, String secretKey) {
        return executeWithMonitoring(
                "encrypt", plainText, encryptType, secretKey, () -> doEncrypt(plainText, encryptType, secretKey));
    }
    ...
    public static String decrypt(String cipherText, EncryptType encryptType, String secretKey) {
        return executeWithMonitoring(
                "decrypt", cipherText, encryptType, secretKey, () -> doDecrypt(cipherText, encryptType, secretKey));
    }

    /**
     * 通用的加密解密执行方法，包含监控和错误处理
     */
    private static String executeWithMonitoring(
            String operation,
            String inputData,
        ...
        try {
            ...
            MONITORING_MANAGER.recordError(
                    operation, encryptType.name(), e.getClass().getSimpleName());
            log.error("数据{}失败: encryptType={}, error={}", operation, encryptType, e.getMessage());
            throw new EncryptException("数据" + operation + "失败: " + e.getMessage(), e);
        }
    }
    ...
    private static void validateSecretKey(String secretKey, String operation) {
        if (StringUtils.isBlank(secretKey)) {
            throw new EncryptException(operation + "密钥不能为空");
        }

        if (secretKey.length() < 16) {
            throw new EncryptException(operation + "密钥长度不足，至少需要16个字符");
        }
    }
    ...
    private static String doEncrypt(String plainText, EncryptType encryptType, String secretKey) throws Exception {
        switch (encryptType) {
            ...
            default:
                throw new EncryptException("不支持的加密类型: " + encryptType);
        }
    }
    ...
    private static String doDecrypt(String cipherText, EncryptType encryptType, String secretKey) throws Exception {
        switch (encryptType) {
            ...
            default:
                throw new EncryptException("不支持的解密类型: " + encryptType);
        }
    }
     ...
     */
    private static String encryptWithCipher(String plainText, EncryptType encryptType, String secretKey)
            throws Exception {
     ...
     */
    private static String decryptWithCipher(String cipherText, EncryptType encryptType, String secretKey)
            throws Exception {
     ...
     * 通用的 Cipher 处理方法
     */
    private static String processCipher(String inputData, EncryptType encryptType, String secretKey, boolean isEncrypt)
            throws Exception {
        ...
        if (transformation.contains("CBC")) {
            return isEncrypt ? encryptWithCBC(cipher, keySpec, inputData) : decryptWithCBC(cipher, keySpec, inputData);
        } else {
            return isEncrypt
