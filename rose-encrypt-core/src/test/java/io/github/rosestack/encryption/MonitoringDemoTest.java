...
class MonitoringDemoTest {
    ...
    void demonstrateMonitoringFeatures() {
        ...
        // 不同算法
        String text1 = EncryptUtils.encrypt("Hello AES", EncryptType.AES, SECRET_KEY);
        String text2 = EncryptUtils.encrypt("Hello DES", EncryptType.DES, SECRET_KEY);
        String text3 = EncryptUtils.encrypt("Hello 3DES", EncryptType.DES3, SECRET_KEY);

        // 不同大小的数据
        String smallData = EncryptUtils.encrypt("Small", EncryptType.AES, SECRET_KEY);
        String mediumData = EncryptUtils.encrypt(
                "This is a medium length text for testing crypto performance and monitoring capabilities",
                EncryptType.AES,
                SECRET_KEY);
        ...
        String largeData = EncryptUtils.encrypt(largeDataBuilder.toString(), EncryptType.AES, SECRET_KEY);

        // 解密操作
        EncryptUtils.decrypt(text1, EncryptType.AES, SECRET_KEY);
        EncryptUtils.decrypt(text2, EncryptType.DES, SECRET_KEY);
        EncryptUtils.decrypt(smallData, EncryptType.AES, SECRET_KEY);

        // 故意触发一些错误
        try {
            EncryptUtils.decrypt("invalid_base64", EncryptType.AES, SECRET_KEY);
        } catch (Exception e) {
            // 预期的错误
        }

        try {
            EncryptUtils.encrypt("test", EncryptType.AES, "short"); // 短密钥
        } catch (Exception e) {
            // 预期的错误
        }
        ...
        Map<String, Long> performanceStats = EncryptMonitorManager.getInstance().getPerformanceStats();
        performanceStats.entrySet().stream()
                .filter(entry ->
                        entry.getKey().contains("success") || entry.getKey().contains("failure"))
        ...
        algorithmStats.forEach((algorithm, count) -> System.out.printf("   %s: %d 次%n", algorithm, count));
        System.out.println();
        ...
        if (errorStats.isEmpty()) {
            ...
            errorStats.forEach((errorType, count) -> System.out.printf("   %s: %d 次%n", errorType, count));
        }
        System.out.println();
        ...
        dataSizeStats.forEach((sizeCategory, count) -> System.out.printf("   %s: %d 次%n", sizeCategory, count));
        System.out.println();
        ...
        successRates.forEach((operation, rate) -> System.out.printf("     %s: %.1f%%%n", operation, rate));
    ...
    void performanceBenchmark() {
        ...
        for (int i = 0; i < iterations; i++) {
            String encrypted = EncryptUtils.encrypt(testData, EncryptType.AES, SECRET_KEY);
            EncryptUtils.decrypt(encrypted, EncryptType.AES, SECRET_KEY);
        }
        ...
        performance.entrySet().stream()
                .filter(entry ->
                        entry.getKey().contains("AES") && entry.getKey().contains("time"))
                .forEach(entry -> {
