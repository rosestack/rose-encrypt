package io.github.rosestack.spring.boot.encrypt;

import static org.assertj.core.api.Assertions.assertThat;

import io.github.rosestack.encrypt.FieldEncryptor;
import io.github.rosestack.encrypt.hash.HashService;
import io.github.rosestack.spring.boot.encrypt.config.CryptoAutoConfig;
...
class CryptoAutoConfigTest {
    ...
    private final ApplicationContextRunner contextRunner =
            new ApplicationContextRunner().withConfiguration(AutoConfigurations.of(CryptoAutoConfig.class));
    ...
    void shouldCreateFieldEncryptorWithDefaultConfig() {
        contextRunner
                .withPropertyValues(
                        "rose.crypto.enabled=true", "rose.crypto.secret-key=MySecretKey12345MySecretKey12345")
                .run(context -> {
    ...
    void shouldNotCreateBeansWhenDisabled() {
        contextRunner.withPropertyValues("rose.crypto.enabled=false").run(context -> {
    ...
    void shouldCreateHashServiceWhenEnabled() {
        contextRunner
                .withPropertyValues(
                        "rose.crypto.enabled=true",
                        "rose.crypto.secret-key=MySecretKey12345MySecretKey12345",
                        "rose.crypto.hash.enabled=true")
                .run(context -> {
    ...
    void shouldNotCreateHashServiceWhenDisabled() {
        contextRunner
                .withPropertyValues(
                        "rose.crypto.enabled=true",
                        "rose.crypto.secret-key=MySecretKey12345MySecretKey12345",
                        "rose.crypto.hash.enabled=false")
                .run(context -> {
                    assertThat(context).doesNotHaveBean(HashService.class);
                });
    }
    ...
    void shouldUseCustomFieldEncryptor() {
        contextRunner
                ...
                .run(context -> {
                    assertThat(context).hasSingleBean(FieldEncryptor.class);
                    assertThat(context.getBean(FieldEncryptor.class)).isInstanceOf(CustomFieldEncryptor.class);
                });
    }
    ...
    void shouldBindConfigurationProperties() {
        contextRunner
                .withPropertyValues(
                        "rose.crypto.enabled=true",
                        "rose.crypto.secret-key=MyTestKey123456789012345678901",
                        "rose.crypto.fail-on-error=false")
                .run(context -> {
                    assertThat(context).hasSingleBean(FieldEncryptor.class);
                    // 这里可以进一步验证配置是否正确应用
                });
    ...
    static class CustomFieldEncryptor implements FieldEncryptor {
        ...
        @Override
        public String encrypt(String plainText, io.github.rosestack.encrypt.enums.EncryptType encryptType) {
            return "custom_encrypted_" + plainText;
        }

        @Override
        public String decrypt(String cipherText, io.github.rosestack.encrypt.enums.EncryptType encryptType) {
