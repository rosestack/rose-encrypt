package io.github.rosestack.spring.boot.encrypt.metrics;
...
public class CryptoMetrics implements MeterRegistryCustomizer<MeterRegistry> {
        ...
        Gauge.builder("crypto.operations.total", this, metrics -> {
                    try {
                        Map<String, Long> stats =
                                EncryptMonitorManager.getInstance().getPerformanceStats();
    ...
    private void registerAlgorithmUsageMetrics() {
        ...
        for (String algorithm : algorithms) {
            Gauge.builder("crypto.algorithm.usage", algorithm, alg -> {
                        try {
                            Map<String, Long> stats =
                                    EncryptMonitorManager.getInstance().getAlgorithmUsageStats();
        ...
        Gauge.builder("crypto.errors.total", this, metrics -> {
                    try {
                        Map<String, Long> errors =
                                EncryptMonitorManager.getInstance().getErrorStats();
        ...
        for (String operation : operations) {
            for (String category : sizeCategories) {
                ...
                Gauge.builder("crypto.data.size.distribution", key, k -> {
                            try {
                                Map<String, Long> stats =
                                        EncryptMonitorManager.getInstance().getDataSizeStats();
