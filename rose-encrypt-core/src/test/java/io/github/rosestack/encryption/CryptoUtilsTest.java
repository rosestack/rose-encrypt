...
import io.github.rosestack.encrypt.exception.EncryptException;
...
class CryptoUtilsTest {
    ...
    void testAESEncryptDecrypt() {
        ...
        // 解密
        String decrypted = EncryptUtils.decrypt(encrypted, EncryptType.AES, TEST_SECRET_KEY);
        assertEquals(TEST_PLAIN_TEXT, decrypted);
    }

    @ParameterizedTest
    @EnumSource(
            value = EncryptType.class,
            names = {"AES", "DES", "DES3"})
    @DisplayName("多种加密算法测试")
    void testMultipleEncryptionTypes(EncryptType encryptType) {
        String encrypted = EncryptUtils.encrypt(TEST_PLAIN_TEXT, encryptType, TEST_SECRET_KEY);
        assertNotNull(encrypted);
        assertNotEquals(TEST_PLAIN_TEXT, encrypted);

        String decrypted = EncryptUtils.decrypt(encrypted, encryptType, TEST_SECRET_KEY);
        assertEquals(TEST_PLAIN_TEXT, decrypted);
    }
    ...
    void testNullAndEmptyValues() {
        // 空字符串
        assertEquals("", EncryptUtils.encrypt("", EncryptType.AES, TEST_SECRET_KEY));
        assertEquals("", EncryptUtils.decrypt("", EncryptType.AES, TEST_SECRET_KEY));

        // null值
        assertNull(EncryptUtils.encrypt(null, EncryptType.AES, TEST_SECRET_KEY));
        assertNull(EncryptUtils.decrypt(null, EncryptType.AES, TEST_SECRET_KEY));
    }

    @Test
    @DisplayName("密钥为空时抛出异常")
    void testEmptySecretKeyThrowsException() {
        assertThrows(EncryptException.class, () -> EncryptUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, ""));

        assertThrows(EncryptException.class, () -> EncryptUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, null));
    }
    ...
    void testSM4EncryptionWithJavaScriptEngine() {
        try {
            String encrypted = EncryptUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.SM4, TEST_SECRET_KEY);
            String decrypted = EncryptUtils.decrypt(encrypted, EncryptType.SM4, TEST_SECRET_KEY);
            assertEquals(TEST_PLAIN_TEXT, decrypted);
    ...
    void testDifferentTextLengths(String text) {
        String encrypted = EncryptUtils.encrypt(text, EncryptType.AES, TEST_SECRET_KEY);
        assertNotNull(encrypted);
    ...
    void testPerformanceStats() {
        ...
        Map<String, Long> emptyStats = EncryptMonitorManager.getInstance().getPerformanceStats();
    ...
    void testBatchEncryptDecrypt() {
        ...
        String[] decrypted = EncryptUtils.decryptBatch(encrypted, EncryptType.AES, TEST_SECRET_KEY);
    ...
    void testEncryptionRandomness() {
        ...
        String encrypted1 = EncryptUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, TEST_SECRET_KEY);
        String encrypted2 = EncryptUtils.encrypt(TEST_PLAIN_TEXT, EncryptType.AES, TEST_SECRET_KEY);

        assertNotEquals(encrypted1, encrypted2, "相同明文的两次加密结果应该不同（由于随机IV）");

        // 但解密结果应该相同
        String decrypted1 = EncryptUtils.decrypt(encrypted1, EncryptType.AES, TEST_SECRET_KEY);
        String decrypted2 = EncryptUtils.decrypt(encrypted2, EncryptType.AES, TEST_SECRET_KEY);
