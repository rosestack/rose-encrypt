 ...
 * 安全的字段加密器实现
 *
 * <p>基于通用加密工具类 CryptoUtils 实现，提供以下特性：
 * <ul>
 *   <li>统一的加密解密逻辑</li>
 *   <li>安全的错误处理机制</li>
 *   <li>完善的日志记录</li>
...
public class DefaultFieldEncryptor implements FieldEncryptor {
    ...
    public String encrypt(String plainText, EncryptType encryptType) {
        ...
        try {
            String result = EncryptUtils.encrypt(plainText, encryptType, secretKey);
    ...
    public String decrypt(String cipherText, EncryptType encryptType) {
        ...
        try {
            String result = EncryptUtils.decrypt(cipherText, encryptType, secretKey);
            ...
            throw new EncryptException("字段解密失败", e);
        }
    }
}
