package io.github.rosestack.spring.boot.encrypt.actuator;

import io.github.rosestack.encrypt.monitor.EncryptMonitorManager;
import io.github.rosestack.spring.boot.encrypt.config.CryptoProperties;
 ...
 * <p>提供加密服务的监控和管理功能：
 * <ul>
 *   <li>GET /actuator/crypto - 获取完整的监控报告</li>
 *   <li>POST /actuator/crypto/clear-cache - 清空缓存和统计</li>
 * </ul>
 *
 * @author chensoul
 * @since 1.0.0
...
public class CryptoEndpoint {
    ...
    public Map<String, Object> crypto() {
        ...
        if (properties.isEnabled()) {
            try {
                Map<String, Object> monitoringReport =
                        EncryptMonitorManager.getInstance().getMonitoringReport();
    ...
    public Map<String, Object> clearCache() {
        ...
        try {
            // 获取清空前的统计
            Map<String, Long> beforeStats = EncryptMonitorManager.getInstance().getPerformanceStats();
            long totalOperationsBefore =
                    beforeStats.values().stream().mapToLong(Long::longValue).sum();
