package io.github.rosestack.encrypt.monitor;
 ...
 */
@Slf4j
public class EncryptMonitorManager {

    /**
     * 单例实例
     */
    private static final EncryptMonitorManager INSTANCE = new EncryptMonitorManager();
     ...
     * 私有构造函数
     */
    private EncryptMonitorManager() {}

    /**
     * 获取单例实例
     */
    public static EncryptMonitorManager getInstance() {
        return INSTANCE;
    }
    ...
    public void recordPerformance(String operation, String encryptType, long startTime, boolean success, int dataSize) {
        ...
        performanceTimes
                .computeIfAbsent(operation + "_" + encryptType + "_total_time", k -> new AtomicLong(0))
                .addAndGet(duration);
        ...
        dataSizeStats
                .computeIfAbsent(operation + "_" + sizeCategory, k -> new AtomicLong(0))
                .incrementAndGet();

        // 记录平均处理时间
        String avgKey = operation + "_" + encryptType + "_avg_time";
        long currentCount =
                performanceCounters.getOrDefault(key, new AtomicLong(0)).get();
        if (currentCount > 0) {
            long totalTime = performanceTimes
                    .getOrDefault(operation + "_" + encryptType + "_total_time", new AtomicLong(0))
                    .get();
    ...
    private Map<String, Double> calculateSuccessRates() {
        ...
        performanceCounters.entrySet().stream()
                ...
                .forEach(successEntry -> {
                    ...
                    long failureCount = performanceCounters
                            .getOrDefault(failureKey, new AtomicLong(0))
                            .get();
    ...
    private long getTotalOperations() {
        return performanceCounters.values().stream().mapToLong(AtomicLong::get).sum();
    }
